...
μεταξύ τους χωρίς γνώση για την υποκειμενική αξία του αγαθού. Αυτό δυσκολεύει τον προσδιορισμό ιδανικού
πονταρίσματος και δημιουργίας βέλτιστης στρατιγικής που μεγιστοποιεί το κέρδος. Παραδείγματα τέτοιων
περιβαλλόντων αποτελούν οι δημοπρασίες στο eBay ή  online διαφημιστικές δημοπρασίες.
...
Η παραπάνω προκληση μπορεί να προσεγγιστεί με το πρόβλημα των Multi-Armed Bandit (MAB) στο οποίο η κάθε πιθανή
προσφορά αντιμετωπίζεται ώς ένα ανεξάρτητος "βραχίονας" μηχανής κουλοχέρη. Σε αυτή τη περίπτωση ο βαθμός μεταμέλειας
κλιμακώνεται γραμμικά με τον αριθμό προσφορών: $O(\sqrt{T|B|})$. Αυτή η τεχνική είναι αργή και δεν εκμεταλλεύεται
τη δομή των δημοπρασιών και της μερική ανατροφοδότησης.

Έρευνες που προσπαθούν να εκμεταλλευτούν την παραπάνω πληροφία είναι αποτελούν οι αλγόριθμοι που βασίζονται σε δημοπρασίες
δεύτερης-τιμής (second-price auctions). Το μειονέκτημα αυτών των αλγορίθμων είναι πως αδυνατούν της προσαρμογής σε γενικευμένες
περιπτώσεις.

Στην περίπτωση που ο πλειοδότης γνωρίζει την αξία του, οι ρυθμοί σύγκλισης είναι πολύ ταχύτεροι. Αυτή η γνώση είναι σπάνια να
υπάρξει στην πράξει.
...
\begin{itemize}[itemsep=0pt, parsep=0pt]
    \item Ο πλειοδότης επιλέγει μια δράση (προσφορά) $b$.
    \item Ο αντίπαλος (adversary) επιλέγει μια συνάρτηση κατανομής $x_t$​ (π.χ. πιθανότητα κλικ) και μια συνάρτηση ανταμοιβής $r_t$
    \item Ο πλειοδότης παρατηρεί τη συνάρτηση $x_t$​ και τη συνάρτηση ανταμοιβής $r_t​(\cdot,o_t​)$ για το συγκεκριμένο αποτέλεσμα $o_t$​ που προέκυψε.
\end{itemize}

Αυτή η προσέγγιση επιτρέπει τη χρήση του αλγορίθμου WIN-EXP, ο οποίος επιτυγχάνει ρυθμούς μεταμέλειας που εξαρτώνται λογαριθμικά από τον
αριθμό των δράσεων και γραμμικά μόνο από τον αριθμό των πιθανών αποτελεσμάτων: $O(\sqrt{T|O|\log(|B|)})$
