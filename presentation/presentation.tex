...
\begin{frame}{H Αμερόληπτη Εκτίμηση Χρησιμότητας}
    \begin{itemize}
        ...
        \item \textbf{Ορισμός (για win-only feedback):}
            ...
            \end{itemize}
        \item \textbf{Ιδιότητα (Λήμμα 3.1):} Η συνάρτηση $\tilde{u}_t(b)$ είναι αμερόληπτη εκτίμηση της πραγματικής αναμενόμενης χρησιμότητας $u_t(b)$, μετατοπισμένη κατά $-1$ ($\mathbb{E}[\tilde{u}_t(b)] = u_t(b) - 1$).
    \end{itemize}
\end{frame}


\subsection{Μεταμέλεια}
\begin{frame}{Μεταμέλεια}
    \begin{itemize}
        \item \textbf{Θεωρητικό Όριο (Theorem 3.2):} Η αναμενόμενη μεταμέλεια για win-only feedback είναι $4\sqrt{T \log(|B|)}$.
        \item \textbf{Γενικευμένο Όριο (Theorem 4.1):} Για $O$ πιθανά αποτελέσματα, η μεταμέλεια είναι $2\sqrt{2T|O| \log(|B|)}$.
        \item \textbf{Σύγκριση με Bandit Αλγορίθμους:}
            \begin{itemize}
                \item Generic Bandit: $O(\sqrt{T|B|})$ (γραμμική εξάρτηση από το μέγεθος του χώρου δράσεων).
                \item WIN-EXP: $O(\sqrt{T \log(|B|)})$ (λογαριθμική εξάρτηση - ταχύτερη σύγκλιση).
            \end{itemize}
        \item \textbf{Σημασία:} Η απόδοση είναι σχεδόν ισοδύναμη με το σενάριο πλήρους πληροφόρησης.
    \end{itemize}
\end{frame}
...
\begin{frame}{Διακριτοποίηση και Lipschitz Συνέχεια}
    \textbf{Πρόβλημα:} Ο χώρος των προσφορών είναι συνεχής $[0, V_{max}]$, αλλά ο αλγόριθμος απαιτεί διακριτά σημεία.
    
    \begin{itemize}
        \item \textbf{Διακριτοποίηση (Binning):} Χωρίζουμε το συνεχές διάστημα σε $K$ επιμέρους διαστήματα (\textbf{bins}).
        \item \textbf{$\Delta$-Lipschitz Συνέχεια:} Αποδεικνύεται ότι η αναμενόμενη χρησιμότητα $u_t(b)$ είναι Lipschitz συνεχής. 
        \begin{itemize}
            \item Εγγυάται «ομαλότητα»: μικρές αλλαγές στο $b$ επιφέρουν μικρές αλλαγές στη χρησιμότητα.
        \end{itemize}
        \item \textbf{Φράγμα Μεταμέλειας (Theorem 5.4):} Η μεταμέλεια στον συνεχή χώρο είναι $O(T^{2/3}\sqrt{\log T})$.
        \item \textbf{Trade-off:} Το σφάλμα προκύπτει από τον συνδυασμό της εκτίμησης του αλγορίθμου και του σφάλματος διακριτοποίησης.
    \end{itemize}
\end{frame}

\subsection{Πρακτική Εφαρμογή}
\begin{frame}{Πρακτική Εφαρμογή και Bid Simulators}
    \textbf{Ερώτημα:} Πού βρίσκουμε τις συναρτήσεις $x_t$ (κατανομή) και $p_t$ (πληρωμή);
    
    \begin{itemize}
        \item \textbf{Η Φύση της $x_t(b)$:} Αντιπροσωπεύει την \textbf{Αθροιστική Συνάρτηση Κατανομής (CDF)} των προσφορών των αντιπάλων.
        \item \textbf{Bid Simulators:} Εργαλεία (π.χ. Google Ads, Bing) παρέχουν έτοιμες αυτές τις καμπύλες (CDF) στους πλειοδότες.
        \item \textbf{Προσέγγιση μέσω Regression:} Αν δεν υπάρχει Simulator, οι συναρτήσεις εκτιμώνται από ιστορικά δεδομένα:
        \begin{itemize}
            \item \textbf{Logistic Regression:} Για την εκτίμηση της πιθανότητας κλικ ($x_t$).
            \item \textbf{Linear Regression:} Για την εκτίμηση της πληρωμής ($p_t$).
        \end{itemize}
        \item \textbf{Robustness:} Ο WIN-EXP αποδίδει καλά ακόμα και με «θορυβώδεις» εκτιμήσεις των συναρτήσεων αυτών.
    \end{itemize}
...
\begin{frame}{Πειράματα}
    \textbf{Προσομοίωση:}
    \begin{itemize}
        \item \textbf{Weighted General Second Price - GSP:} Ρυθμός "κλικ" (CTR) $U[0.1,1]$, $U[0.3,1]$ και $U[0.5,1]$
        \item \textbf{Ιδιότητες:} 5.000 γύρους, 20 πλειοδότες, 3 θέσεις (slots) διαφήμισης.
        \item \textbf{Τύποι αντιπάλων:} 1. Στοχαστικοί αντίπαλοι, 2. Προσαρμοστική αντίπαλοι EXP3, 3. Προσαρμοστική αντίπαλοι WIN-EXP.
    \end{itemize}

    \textbf{Αποτελέσματα:}
    \begin{itemize}
        \item O WIN-EXP υπερέχει σταθερά του EXP3 και στις τρεις παραπάνω κατηγορίες.
        \item Η απόδοση του WIN-EXP είναι σταθερή σε χώρο με πολλές προσφορές.
        \item Ο WIN-EXP δείχνει ανθεκτικότητα (robustness) σε δεδομένα με θόρυβο (noisy data).
    \end{itemize}
\end{frame}
...
\begin{frame}{Σύνοψη}
    \begin{itemize}
        \item \textbf{Βασικό Εύρημα:} Η άγνωστη αξία στις δημοπρασίες είναι μια «καλοήθης» (benign) μορφή ελλιπούς πληροφορίας.
        \item \textbf{Αποδοτικότητα:} Ο WIN-EXP επιτυγχάνει ρυθμούς μεταμέλειας σχεδόν ισοδύναμους με το σενάριο πλήρους πληροφόρησης.
        \item \textbf{Aνθεκτικότητα:} Τα αποτελέσματα παραμένουν ισχυρά ακόμα και με θορυβώδη δεδομένα.
        \item \textbf{Lipschitz:} <INSERT HERE RESULTS FROM 5, 6>
        \item \textbf{Μελλοντική Έρευνα:} Επέκταση σε περιβάλλοντα όπου η αξία αποκαλύπτεται με καθυστέρηση ή εξαρτάται από τον ανταγωνισμό.
    \end{itemize}
\end{frame}
